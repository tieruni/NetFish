...
- (void)viewDidLoad {
    ...
    self.IMW4 = [UIImageView new];
    self.IMW4 = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, UI_SCREEN_W, 200)];
    _IMW4.backgroundColor =[UIColor redColor];
    _IMW4.image = [UIImage imageNamed:@"Image66"];
    [_tableviewTechnology addSubview:_IMW4];
    //将图片视图塞进headerview
    _tableviewTechnology.tableHeaderView = self.IMW4 ;
}
...
-(void)requestTechnologyData{
    ...
    PFQuery *query = [PFQuery queryWithClassName:@"technologynews"];
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray * _Nullable objects, NSError * _Nullable error) {
        ...
        if (!error) {
            ...
            _objectForShowTechnology = [NSMutableArray arrayWithArray:objects];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return  _objectForShowTechnology.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *identy = @"CustomCell4";
    if (!nib) {
        //        FirstTableViewCell *cell =[[FirstTableViewCell alloc]initWithFrame:CGRectMake(0, 200, UI_SCREEN_W, 110)];
        nib = [UINib nibWithNibName:@"FourVCXib" bundle:nil];
        [tableView registerNib:nib forCellReuseIdentifier:identy];
        NSLog(@"我是从nib过来的，%ld",indexPath.row);
    }
    FourTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identy];
    //    NSUInteger row = [indexPath row];
    
    PFObject *obj = _objectForShowTechnology[indexPath.row];
    NSString *title = obj[@"title4"];
    cell.FourVCTechnologyTitle.text = title;
    
    NSString *new = obj[@"news4"];
    cell.FourVCTechnologyTxtLab.text = new;
    
    PFFile *photofile = obj[@"photo4"];
    NSString *photoUrlStr = photofile.url;
    NSURL *photoUrl = [NSURL URLWithString:photoUrlStr];
    [cell.FourVCTechnologyImageView sd_setImageWithURL:photoUrl placeholderImage:[UIImage imageNamed:@"Image77"]] ;
    //    if (row%2 == 0) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(nonnull NSIndexPath *)indexPath{
    ...
    //根据上述行数获取该行所对应的数据
    PFObject *newDetail = _objectForShowTechnology [indexPath.row];
    //获得将要跳转到的页面的实例
